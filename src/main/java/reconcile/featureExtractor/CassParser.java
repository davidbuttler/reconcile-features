      ...
      if (!inSentence) {
        ...
        for (int w = 0; w < words.length; w++) {
          if (pAssignment.matcher(words[w]).matches() && w < 1) {
            ...
            String tokText = Annotation.getAnnotText(tok, text);
            if (!tokText.equalsIgnoreCase(word) && !tokText.equals("]"))
              throw new RuntimeException(Annotation.getAnnotText(tok, text) + " vs. " + word);
    ...
    throws IOException
{
  ...
  for (Annotation sentence : sentSet) {
    ...
    for (Annotation tok : toks) {
      tempFile.write(Annotation.getAnnotText(tok, text).replaceAll("]", ")") + "\t" + tok.getType() + "\n");
    }
    tempFile.write("</s>\n");
  }
  tempFile.flush();
