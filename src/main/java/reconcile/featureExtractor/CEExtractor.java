...
public static boolean containsBreak(int start, int end, AnnotationSet parse, AnnotationSet ne, String text)
{
  ...
  for (Annotation o : overlap) {
    ...
    if (o.getType().equals("CC") && !"or".equalsIgnoreCase(Annotation.getAnnotText(o, text)) && !ne.coversSpan(o)
...
public static boolean containsConj(Annotation a, AnnotationSet parse, String text)
{
  ...
  for (Annotation o : overlap) {
    if (o.getType().equals("CC") && !"or".equalsIgnoreCase(Annotation.getAnnotText(o, text))) return true;
  }
  return false;
}
...
{
  if (!p.getType().equals("NP") && Annotation.getAnnotText(p, text).endsWith("ing")) {
    System.out.println("Gerund " + Annotation.getAnnotText(p, text));
    return true;
  }
  return false;
}
...
public static boolean isJunk(Annotation a, String text)
{
  ...
  String annotText = Annotation.getAnnotText(a, text);
...
public static Annotation trimNP(Annotation np, AnnotationSet parse, String text)
{
  ...
  for (Annotation p : contained) {
    if ((p.getType().equals("POS") || p.getType().equals("\"") || Annotation.getAnnotText(p, text).matches(
        "\\(|\\)|\\\"|\\.|\\,"))
  ...
  String anText = Annotation.getAnnotText(result, text);
  ...
  while (anText.length() > 0 && anText.substring(anText.length() - 1).matches("\\(|\\)|\\\"|\\,|\\s|\\_")) {// &&
    ...
    endOffset--;
    anText = Annotation.getAnnotText(result.getStartOffset(), endOffset, text);
  }
  ...
  anText = Annotation.getAnnotText(result, text);
  int startOffset = result.getStartOffset();
  while (anText.length() > 0 && anText.substring(0, 1).matches("\\(|\\)|\\\"|\\,|\\s|\\_")) {// &&
                                                                                             // !anText.substring(anText.length()-1).matches("\\.")){
    startOffset++;
    anText = Annotation.getAnnotText(startOffset, result.getEndOffset(), text);
  }
    ...
    throws IOException
{
  ...
  for (Annotation curr : basenp) {
    ...
    fw.write(Annotation.getAnnotTextClean(curr, text) + "\n");
  }
