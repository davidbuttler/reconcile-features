...
public boolean isNP(Annotation an, String text)
{
	try {
		...
		if (FeatureUtils.memberArray(type, SyntaxUtils.NPType)&&!FeatureUtils.memberArray(Annotation.getAnnotText(an, text),HERE)) return true;
		if (type.equalsIgnoreCase("WHNP") && 
				FeatureUtils.memberArray(Annotation.getAnnotText(an, text), REL_POS_PRONOUNS))
			return true;
