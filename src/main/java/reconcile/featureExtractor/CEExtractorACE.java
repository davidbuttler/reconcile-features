...
public boolean isNoun(Annotation a, String text)
{
  ...
  // return true;
  if (type.startsWith("NN") && FeatureUtils.isCapitalized(Annotation.getAnnotText(a, text))) return true;
  if (type.equals("JJ") && FeatureUtils.isCapitalized(Annotation.getAnnotText(a, text))) return true;
...
public boolean isNP(Annotation an, String text)
{
  try {
    ...
    if (type.equalsIgnoreCase("WHNP") && !"that".equalsIgnoreCase(Annotation.getAnnotText(an, text))) return true;
