...
public NamedEntityStanford() {
  ...
  if (model_names.length == 0) {
    ...
    model_names = new String[] { "edu/stanford/nlp/models/ner/all.3class-distsim.crf.ser.gz" };
  }
  ...
  for (String modelName : model_names) {
    ...
    try {
      InputStream res = this.getClass().getClassLoader().getResourceAsStream(modelName);
      if (res != null) {
